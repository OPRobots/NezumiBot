...
#define MAX_WAIT_ITERS 20000u

int16_t sensors_distance_offset[NUM_SENSORS] = {0,0,0,0};
...
volatile uint16_t sensors_distance[NUM_SENSORS];

const float ln_lookup[LOG_LINEARIZATION_TABLE_SIZE] = {
...
/* Valores obtenidos con el script de caracterizacion de sensores
...
*/
void set_sensors_robot_calibration(void) {
      sensors_distance_calibrations[SENSOR_FRONT_LEFT].a = 3.452;
      sensors_distance_calibrations[SENSOR_FRONT_LEFT].b = 0.449;
      sensors_distance_calibrations[SENSOR_FRONT_LEFT].c = 118.207;

      sensors_distance_calibrations[SENSOR_LEFT].a = 2.538;
      sensors_distance_calibrations[SENSOR_LEFT].b = 0.336;
      sensors_distance_calibrations[SENSOR_LEFT].c = 103.665;

      sensors_distance_calibrations[SENSOR_RIGHT].a = 3.058;
      sensors_distance_calibrations[SENSOR_RIGHT].b = 0.407;
      sensors_distance_calibrations[SENSOR_RIGHT].c = 130.457;

      sensors_distance_calibrations[SENSOR_FRONT_RIGHT].a = 3.190;
      sensors_distance_calibrations[SENSOR_FRONT_RIGHT].b = 0.414;
      sensors_distance_calibrations[SENSOR_FRONT_RIGHT].c = 141.043;
}
...
void front_sensors_calibration(void) {
  ...
  sensors_distance_offset[SENSOR_FRONT_LEFT] = 0;
  sensors_distance_offset[SENSOR_FRONT_RIGHT] = 0;
  delay(5);

  for (int i = 0; i < SENSOR_FRONT_CALIBRATION_READINGS; i++) {
    left_temp += sensors_distance[SENSOR_FRONT_LEFT];
    right_temp += sensors_distance[SENSOR_FRONT_RIGHT];
    delay(5);
  }
  ...
  sensors_distance_offset[SENSOR_FRONT_LEFT] = left_offset;
  sensors_distance_offset[SENSOR_FRONT_RIGHT] = right_offset;
}
...
void side_sensors_calibration(void) {
  ...
  sensors_distance_offset[SENSOR_LEFT] = 0;
  sensors_distance_offset[SENSOR_RIGHT] = 0;
  delay(5);

  for (int i = 0; i < SENSOR_SIDE_CALIBRATION_READINGS; i++) {
    left_temp += sensors_distance[SENSOR_LEFT];
    right_temp += sensors_distance[SENSOR_RIGHT];
    delay(5);
  }
  ...
  sensors_distance_offset[SENSOR_LEFT] = left_offset;
  sensors_distance_offset[SENSOR_RIGHT] = right_offset;
}
...
void adc_to_distance_loginv(void) {
    for (uint8_t sensor = 0; sensor < NUM_SENSORS; sensor++) {
        uint16_t sensor_value = sensors_raw[sensor];

        int32_t corrected = (int32_t)sensor_value + (int32_t)sensors_distance_calibrations[sensor].c;
        int32_t ln_index = corrected / LOG_LINEARIZATION_TABLE_STEP;

        if (ln_index < 0) {
            ln_index = 0;
        } else if (ln_index >= (int32_t)LOG_LINEARIZATION_TABLE_SIZE) {
            ln_index = LOG_LINEARIZATION_TABLE_SIZE - 1;
        }

        float ln = ln_lookup[ln_index];

        /* --- Fórmula de caracterización: distance_mm = (a / ln - b) * 1000 --- */
        float a = sensors_distance_calibrations[sensor].a;
        float b = sensors_distance_calibrations[sensor].b;
        int32_t new_sensor_distance = (int32_t)((a / ln - b) * 1000.0f + 0.5f); // redondeo

        if (new_sensor_distance < 0) {
            new_sensor_distance = 0;
        }

        /* --- Ajuste geométrico según la posición del sensor --- */
        switch (sensor) {
        ...
        /* --- Aplicar offset calibrado y asegurar no negativo --- */
        //new_sensor_distance += sensors_distance_offset[sensor];
        if (new_sensor_distance < 0) new_sensor_distance = 0;

        sensors_distance[sensor] = (uint16_t)new_sensor_distance;
    }
}



// // Funciones para calibrar los sensores, descomentar para volver a calibrar

// uint16_t sensor_get_average(uint8_t sensor, uint16_t n_samples) {
//     uint32_t sum = 0;

//     for (uint16_t i = 0; i < n_samples; i++) {
//         sum += sensors_raw[sensor];
//     }
...
// // Buffer circular para un solo sensor a la vez
// static uint16_t sensor_buffer[N_SAMPLES];
// static uint32_t buffer_sum = 0;
// static uint16_t buffer_index = 0;


// // Inicializa el buffer con el valor actual del sensor
// void sensor_buffer_init(uint8_t sensor) {
//     uint16_t initial_value = sensors_raw[sensor];
//     buffer_sum = 0;
//     for (uint16_t i = 0; i < N_SAMPLES; i++) {
//         sensor_buffer[i] = initial_value;
//         buffer_sum += initial_value;
//     }
//     buffer_index = 0;
// }

// // Devuelve la media de las últimas N_SAMPLES lecturas del sensor
// uint32_t sensor_get_average_wait_new(uint8_t sensor) {
//     if (sensor >= NUM_SENSORS) return 0;

//     uint16_t current_value = sensors_raw[sensor];

//     // Sustituye el valor viejo por el nuevo en el buffer circular
//     buffer_sum -= sensor_buffer[buffer_index];
//     sensor_buffer[buffer_index] = current_value;
//     buffer_sum += current_value;

//     // Avanza índice circular
//     buffer_index = (buffer_index + 1) % N_SAMPLES;

//     return buffer_sum / N_SAMPLES;
// }
...
uint16_t get_sensor_calibrated(enum SENSORS index) { // devuelve 0 en las zonas que el sensor tiene ruido (cerca - lejos)  
    ...
    switch (enUso) {
      case Antiguos:
        valor_sensor = get_sensor_raw(index);
        break;
      case Caracterizados:
        valor_sensor = sensors_distance[index];
        break;
    }  
...
void actualizar_bool(int i_sensor){
  if(i_sensor == SENSOR_FRONT_LEFT || i_sensor == SENSOR_FRONT_RIGHT){ // revisar media sensores frontales
    sensor_filtrado[0] = (sensor_filtrado[0] + sensor_filtrado[3])/2;
    sensor_bool[0] = sensor_filtrado[0] > UMBRAL_DETECCION_FRONTAL;
  }else if(i_sensor == SENSOR_LEFT ){
...
void filtro_media(void) {
  for (int i_sensor = 0; i_sensor < NUM_SENSORS; i_sensor++){
    ...
    if(i_sensor == SENSOR_FRONT_LEFT || i_sensor == SENSOR_FRONT_RIGHT){
      sensor_filtrado[0] = (sensor_filtrado[0] + sensor_filtrado[3])/2;
    }
...
void filtro_media_paso_bajo_1(void){
  ...
  for (int i_sensor = 0; i_sensor < NUM_SENSORS; i_sensor++){
    ...
    sensor_filtrado_paso_bajo[0][i_sensor] = 0.9*sensor_filtrado_paso_bajo[1][i_sensor] + 0.05*sensor_raw[0][i_sensor] + 0.05*sensor_raw[1][i_sensor]; // magnitud 5 //antes 0.969 0.0155 0.0155
